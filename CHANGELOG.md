...
- **TOML config serialization crashed on `None` fields** — `tomli_w` (the real TOML writer) raises on `None`-valued config fields (e.g. `SACConfig.target_entropy=None`); `None` values are now stripped before dumping (TOML has no null type; `from_dict` restores them from dataclass defaults on read). Fixes 4 CI test failures.
