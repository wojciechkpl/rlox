...
Imports: stdlib + verifiers + httpx + datasets only. No torch, no rlox top-level package.

Dual-context sibling import: works both when python/rlox/agentic/ is injected
directly onto sys.path (test context) and when loaded as rlox.agentic.* (prod).
"""
...
# Dual-context sibling import — survives both test (plain) and prod (relative) modes.
try:
    from . import adversarial_corpus as _ac
except ImportError:
    import adversarial_corpus as _ac  # type: ignore[no-redef]
...
def load_environment(config: RloxVerifierConfig) -> vf.Environment:
